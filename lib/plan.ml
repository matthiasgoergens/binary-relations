...
(* ------------------------------------------------------------------ *)
(* The four-parameter planner                                          *)
(* ------------------------------------------------------------------ *)

(* The same three passes over {!Symbolic.General} trees. The logic is
   unchanged; the two places the two-parameter planner conjured a value out of
   thin air now need a comparator from somewhere:

   - [comp]'s absorbing [Bot] cases must rebuild [Bot] at a NEW range type,
     so they read the comparator off the other operand with
     {!Symbolic.General.ca_of}/{!Symbolic.General.cb_of} — total, because
     every node carries or computes its comparators;
   - [plan_chain]'s empty-chain case has nothing to read one from. It is
     unreachable — [flatten] of a [Comp] yields at least two elements, and the
     recursion only descends into non-empty rests — so it announces rather
     than guessing. *)
module General = struct
  open Symbolic.General

  let comp : type a ac b bc c cc.
      (a, ac, b, bc) t -> (b, bc, c, cc) t -> (a, ac, c, cc) t =
   fun x y ->
    match (x, y) with
    | Id _, _ -> y
    | _, Id _ -> x
    | Bot (ca, _), _ -> Bot (ca, cb_of y)
    | _, Bot (_, cb) -> Bot (ca_of x, cb)
    | _ -> Comp (x, y)

  let meet_ : type a ac b bc. (a, ac, b, bc) t -> (a, ac, b, bc) t -> (a, ac, b, bc) t =
   fun x y -> match (x, y) with Bot _, _ -> x | _, Bot _ -> y | _ -> Meet (x, y)

  let join_ : type a ac b bc. (a, ac, b, bc) t -> (a, ac, b, bc) t -> (a, ac, b, bc) t =
   fun x y -> match (x, y) with Bot _, _ -> y | _, Bot _ -> x | _ -> Join (x, y)

  (* Converse distributes over everything in sight, and at a leaf it is free.
     So the right normal form is to have converses only on leaves. *)
  let rec conv : type a ac b bc. (a, ac, b, bc) t -> (b, bc, a, ac) t =
   fun t ->
    match t with
    | Conv x -> x
    | Id _ -> t
    ...
    | Plus x -> Plus (conv x)
    | Star x -> Star (conv x)
    | Where _ -> t (* a coreflexive is its own converse *)
    | Leaf r -> Leaf (Relation.General.converse r) (* free: the indexes are shared *)
    | _ -> Conv t

  let rec simplify : type a ac b bc. (a, ac, b, bc) t -> (a, ac, b, bc) t =
   fun t ->
    match t with
    | Comp (x, y) -> comp (simplify x) (simplify y)
    | Conv x -> conv (simplify x)
    ...
    | MeetComp (x, y, z) -> MeetComp (simplify x, simplify y, simplify z)
    | MeetComp3 (x, m, y, z) -> MeetComp3 (simplify x, simplify m, simplify y, simplify z)
    | Id _ | Bot _ | Fst _ | Snd _ | Where _ | Fn _ | Leaf _ -> t

  (* A chain makes the intermediate types existential, which is what lets the
     dynamic program cut it anywhere. *)
  type ('a, 'ac, 'b, 'bc) chain =
    | Nil : ('a, 'ac, 'a, 'ac) chain
    | Cons : ('a, 'ac, 'b, 'bc) t * ('b, 'bc, 'c, 'cc) chain -> ('a, 'ac, 'c, 'cc) chain

  type ('a, 'ac, 'c, 'cc) split =
    | Split : ('a, 'ac, 'b, 'bc) chain * ('b, 'bc, 'c, 'cc) chain -> ('a, 'ac, 'c, 'cc) split

  let rec flatten : type a ac b bc c cc.
      (a, ac, b, bc) t -> (b, bc, c, cc) chain -> (a, ac, c, cc) chain =
   fun t rest -> match t with Comp (x, y) -> flatten x (flatten y rest) | _ -> Cons (t, rest)

  let rec chain_length : type a ac b bc. (a, ac, b, bc) chain -> int =
   fun ch -> match ch with Nil -> 0 | Cons (_, rest) -> 1 + chain_length rest

  let rec split_at : type a ac c cc. int -> (a, ac, c, cc) chain -> (a, ac, c, cc) split =
   fun n ch ->
    match (n, ch) with
    | 0, _ -> Split (Nil, ch)
    | _, Nil -> Split (Nil, Nil)
  ...
  (** What the planner knows about a (sub)expression. For a leaf every field is
      exact; for an intermediate they are estimates built on estimates, and the
      uniformity assumption compounds along the chain. See the note on the
      two-parameter {!Plan.info} — it applies verbatim. *)
  type info = { card : float; dom : float; rng : float }

  let info_of_relation r =
    let s = Relation.General.stats r in
    {
      card = Float.of_int s.card;
      dom = Float.of_int s.domain_size;
      rng = Float.of_int s.range_size;
  ...
  let info_of : type a ac b bc. (a, ac, b, bc) t -> info option = function
    | Leaf r -> Some (info_of_relation r)
    | _ -> None

  (* The textbook join-size estimate under uniformity. See the two-parameter
     [combine] for why this is an order of magnitude out in both directions
     under skew — the guard below asks it only for an ordering against an
     exact leaf count. *)
  let combine x y =
    let denom = Float.max 1.0 (Float.max x.rng y.dom) in
    let card = x.card *. y.card /. denom in
  ...
  (* Cost in tuples touched: both operands are scanned or probed, and the
     result is built. *)
  let step_cost x y = x.card +. y.card +. (combine x y).card
  ...
  let rec build : type a ac c cc. bracket -> (a, ac, c, cc) chain -> (a, ac, c, cc) t =
   fun br ch ->
    match br with
    (* A bracket is built for a chain of a known length, so these two cases
       are unreachable. They raise rather than quietly falling back: a planner
       that silently emits a different plan from the one it costed would still
       be correct, and would make every measurement a lie. *)
    | BLeaf -> (
      match ch with
      | Cons (x, Nil) -> x
      | Nil -> failwith "Plan.General.build: bracket and chain disagree (empty segment)"
      | Cons (_, _) -> failwith "Plan.General.build: bracket and chain disagree (segment too long)")
    | BSplit (n, bl, brr) ->
      let (Split (l, r)) = split_at n ch in
      Comp (build bl l, build brr r)

  (* Bind a coreflexive to whatever precedes it, before the interval DP runs.
     Essentially always at least as good, because a filter only ever shrinks
     what it is composed with — and it needs no estimate to justify. *)
  let rec bind_coreflexives : type a ac b bc. (a, ac, b, bc) chain -> (a, ac, b, bc) chain =
   fun ch ->
    match ch with
    | Nil -> Nil
    | Cons (e1, Cons ((Where _ as e2), rest)) ->
      bind_coreflexives (Cons (Comp (e1, e2), rest))
    | Cons (e1, Cons ((Meet (Id _, _) as e2), rest)) ->
      bind_coreflexives (Cons (Comp (e1, e2), rest))
    | Cons (e1, Cons ((Meet (_, Id _) as e2), rest)) ->
      bind_coreflexives (Cons (Comp (e1, e2), rest))
    | Cons (e, rest) -> Cons (e, bind_coreflexives rest)

  let rec plan_chain : type a ac b bc. (a, ac, b, bc) chain -> (a, ac, b, bc) t =
   fun ch ->
    let rec run_length : type x xc y yc. (x, xc, y, yc) chain -> int = function
      | Nil -> 0
      | Cons (x, rest) -> ( match info_of x with None -> 0 | Some _ -> 1 + run_length rest)
    in
    let rec infos_of : type x xc y yc. (x, xc, y, yc) chain -> int -> info list =
     fun ch n ->
      if n = 0 then []
      else match ch with
        | Nil -> []
    ...
    match ch with
    | Nil ->
      (* Unreachable: [flatten] of a [Comp] yields at least two elements and
         the recursion only descends into non-empty rests. There is no
         comparator in hand to build [id] with, and guessing is worse. *)
      failwith "Plan.General.plan_chain: empty composition chain"
    | Cons (x, Nil) -> x
    | Cons (x, rest) ->
      let p = run_length ch in
      if p >= 2 then begin
  ...
  let rec reassociate : type a ac b bc. (a, ac, b, bc) t -> (a, ac, b, bc) t =
   fun t ->
    match t with
    | Comp _ ->
      let rec descend : type x xc y yc. (x, xc, y, yc) chain -> (x, xc, y, yc) chain = function
        | Nil -> Nil
        | Cons (e, rest) -> Cons (reassociate e, descend rest)
      in
      plan_chain (bind_coreflexives (descend (flatten t Nil)))
  ...
  (** The estimated cost of a tree as written, in tuples touched. *)
  let rec estimate : type a ac b bc. (a, ac, b, bc) t -> info option * float =
   fun t ->
    match t with
    | Leaf r -> (Some (info_of_relation r), 0.0)
    | Comp (x, y) -> (
  ...
  (* [(x >> y) ∧ z] without building [x >> y] — a win exactly when [z] is
     smaller than the intermediate it would otherwise discard, so the rewrite
     is guarded. The guard asks the cost model only for an ordering against an
     exact leaf count, which is a far weaker demand than an absolute
     estimate. *)
  let fuse_meet p q z =
    match (fst (estimate (Comp (p, q))), fst (estimate z)) with
    | Some ipq, Some iz when Float.( <= ) iz.card ipq.card -> MeetComp (p, q, z)
  ...
  (* Same guard as the two-way case. *)
  let fuse_meet3 p q r z =
    match (fst (estimate (Comp (Comp (p, q), r))), fst (estimate z)) with
    | Some ipqr, Some iz when Float.( <= ) iz.card ipqr.card -> MeetComp3 (p, q, r, z)
    | _ -> Meet (Comp (Comp (p, q), r), z)

  let rec fuse : type a ac b bc. (a, ac, b, bc) t -> (a, ac, b, bc) t =
   fun t ->
    match t with
    | Comp ((Where _ as c), Fork (p, q)) -> Fork (fuse (Comp (c, p)), fuse (Comp (c, q)))
    | Comp ((Meet (Id _, _) as c), Fork (p, q)) -> Fork (fuse (Comp (c, p)), fuse (Comp (c, q)))
    | Comp ((Meet (_, Id _) as c), Fork (p, q)) -> Fork (fuse (Comp (c, p)), fuse (Comp (c, q)))
    | Meet (Comp (Comp (p, q), r), z) -> fuse_meet3 (fuse p) (fuse q) (fuse r) (fuse z)
    | Meet (Comp (p, Comp (q, r)), z) -> fuse_meet3 (fuse p) (fuse q) (fuse r) (fuse z)
    | Meet (z, Comp (Comp (p, q), r)) -> fuse_meet3 (fuse p) (fuse q) (fuse r) (fuse z)
    | Meet (z, Comp (p, Comp (q, r))) -> fuse_meet3 (fuse p) (fuse q) (fuse r) (fuse z)
  ...
  (** Where the cost model cannot see. *)
  let blind_spots : type a ac b bc. (a, ac, b, bc) t -> string list =
   fun t ->
    let acc = ref [] in
    let add s = acc := s :: !acc in
    let rec go : type x xc y yc. (x, xc, y, yc) t -> unit = function
      | Where (n, _, _) ->
        if Symbolic.node_is_opaque n then add ("opaque predicate in where(" ^ Symbolic.string_of_node n ^ ")")
        else add ("unknown selectivity: where(" ^ Symbolic.string_of_node n ^ ")")
      | Fn _ -> add "host function: fn"
      | Id _ | Bot _ | Fst _ | Snd _ | Leaf _ -> ()
      | Conv x -> go x
      | Plus x -> add "fixpoint: closure size is not predicted"; go x
      | Star x -> add "fixpoint: closure size is not predicted"; go x
      | Group x -> go x
  ...
  let explain t =
    ...
    String.concat ~sep:"\n"
      ([
         "  as written : " ^ Symbolic.General.to_string t;
         Printf.sprintf "  estimated  : %.0f tuples" (estimated_cost t);
         "  planned    : " ^ Symbolic.General.to_string o;
         Printf.sprintf "  estimated  : %.0f tuples" (estimated_cost o);
       ]
      @
      match spots with
